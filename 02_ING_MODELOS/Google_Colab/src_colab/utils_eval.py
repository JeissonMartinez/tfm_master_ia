        ...
        for b in range(batch_size):
            ...
            if valid_mask.any():
                ...
                if anchors is not None:
                    bboxes = _decode_bbox_with_anchors(bboxes, anchors[valid_mask])
...
def _decode_bbox_with_anchors(pred_offsets: np.ndarray, anchors: np.ndarray) -> np.ndarray:
    """Decode offset predictions back to absolute boxes using anchors."""
    cx_a = (anchors[:, 0] + anchors[:, 2]) / 2
    cy_a = (anchors[:, 1] + anchors[:, 3]) / 2
    w_a = anchors[:, 2] - anchors[:, 0]
    h_a = anchors[:, 3] - anchors[:, 1]

    cx = pred_offsets[:, 0] * w_a + cx_a
    cy = pred_offsets[:, 1] * h_a + cy_a
    w = np.exp(pred_offsets[:, 2]) * w_a
    h = np.exp(pred_offsets[:, 3]) * h_a
...
def _compute_map(
    ...
    # global P/R/F1
    total_tp = sum(1 for d in detections for gt in ground_truths
                   if d[0] == gt[0] and d[1] == gt[1])
