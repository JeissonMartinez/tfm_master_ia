...
def _parse_tfrecord_fn(example_proto, img_size: int, max_objects: int = 50):
    ...
    Supports two TFRecord layouts:
      - **Custom** (write_tfrecord): image/boxes, image/class_ids, image/num_objects
      - **Roboflow**: image/object/bbox/{xmin,ymin,xmax,ymax},
                      image/object/class/label (1-indexed)

    Boxes are always returned as [xc, yc, w, h] normalised.
    """
    import tensorflow as tf

    # --- Try Custom format first (has image/num_objects) ----------------
    custom_desc = {
        "image/encoded": tf.io.FixedLenFeature([], tf.string, default_value=""),
        "image/num_objects": tf.io.FixedLenFeature([], tf.int64, default_value=-1),
        "image/boxes": tf.io.VarLenFeature(tf.float32),
        "image/class_ids": tf.io.VarLenFeature(tf.int64),
        # Roboflow fields (may be absent → defaults/empty)
    ...
    parsed = tf.io.parse_single_example(example_proto, custom_desc)
    ...
    def _parse_roboflow():
        ...
        xc = xmin + w / 2.0
        yc = ymin + h / 2.0
        boxes = tf.stack([xc, yc, w, h], axis=1)  # [N, 4]
        # Roboflow labels are 1-indexed → need 0-indexed
        class_ids = tf.cast(
            tf.sparse.to_dense(parsed["image/object/class/label"]), tf.int32
        ) - 1
        num_obj = tf.shape(boxes)[0]
        return boxes, class_ids, num_obj

    boxes, class_ids, num_obj = tf.cond(has_custom, _parse_custom, _parse_roboflow)
...
def _resolve_tfrecord_path(dataset_dir: Path, split: str) -> Optional[Path]:
    """Find the TFRecord file for a given split.

    Supports two layouts:
      - Flat:     {dataset_dir}/{split}.tfrecord
      - Roboflow: {dataset_dir}/{split}/*.tfrecord  (or valid/ for val)
    """
    # 1. Flat layout
    flat = dataset_dir / f"{split}.tfrecord"
    if flat.exists():
        return flat

    # 2. Roboflow layout (uses 'valid' instead of 'val')
    alt_name = "valid" if split == "val" else split
    subdir = dataset_dir / alt_name
    if subdir.is_dir():
        tfrs = list(subdir.glob("*.tfrecord"))
        if tfrs:
            return tfrs[0]

    # 3. Try subfolder with original split name too
    subdir2 = dataset_dir / split
    if subdir2.is_dir():
        tfrs = list(subdir2.glob("*.tfrecord"))
        if tfrs:
            return tfrs[0]
...
def verify_tfrecord_dataset(dataset_dir: str | Path) -> Dict[str, Any]:
    ...
    Supports both flat ({split}.tfrecord) and Roboflow ({split}/*.tfrecord).
    metadata.json is optional (a warning is shown if absent).
    """
    dataset_dir = Path(dataset_dir)
    result: Dict[str, Any] = {"valid": True, "splits": {}, "issues": []}
    ...
    if meta_path.exists():
        ...
        result["issues"].append(
            f"metadata.json no encontrado en {dataset_dir} (no bloqueante)"
        )
        meta = None

    for split in ("train", "val", "test"):
        tfr = _resolve_tfrecord_path(dataset_dir, split)
        info: Dict[str, Any] = {"exists": False, "num_samples": 0, "path": None}
        if tfr is not None:
            info["exists"] = True
            info["path"] = str(tfr)
            try:
            ...
            result["issues"].append(f"Falta TFRecord para split '{split}'")
            result["valid"] = False
        result["splits"][split] = info
...
def create_mobilenet_pipeline(
    ...
    ----------
    dataset_dir : path
        Root directory containing TFRecords (flat or Roboflow layout).
    split : str
    ...
    img_size = imgsz
    augmentation_level = augment_level

    # Resolve TFRecord path for this split
    tfrecord_path = _resolve_tfrecord_path(Path(dataset_dir), split)
    if tfrecord_path is None:
        raise FileNotFoundError(
            f"No se encontró TFRecord para split '{split}' en {dataset_dir}"
        )
    log(f"📂 Pipeline {split}: {tfrecord_path}")
...
def generate_anchors(
    ...
    # Aliases used by the notebook widgets:
    ...
    Accepts *either* the canonical parameter names (``feature_map_size``,
    ``scales``, ``aspect_ratios``) **or** the widget-friendly aliases
    (``imgsz``, ``sizes``, ``ratios``).

    When ``imgsz`` is given and ``feature_map_size`` is still at its
    default, the feature map size is derived as ``imgsz // 32``.

    Returns:
        Array of shape (H*W*S*A, 4) in normalised [cx, cy, w, h].
    """
    # Resolve aliases
    if sizes is not None and scales is None:
        scales = sizes
    if ratios is not None and aspect_ratios is None:
        aspect_ratios = ratios
    if imgsz is not None and feature_map_size == 7:
        feature_map_size = max(imgsz // 32, 1)
...
def get_class_distribution(
    ...
    if is_yolo_family(model_family):
        ...
        # Fallback: iterate TFRecord (resolve path flexibly)
        tfr = _resolve_tfrecord_path(dataset_dir, split)
        if tfr is None:
            log(f"⚠️ No se encontró TFRecord para split '{split}'")
            return {n: 0 for n in class_names}
        return _count_classes_tfrecord(tfr, class_names)
...
def _count_classes_tfrecord(
    ...
    Supports both custom (image/class_ids) and Roboflow
    (image/object/class/label, 1-indexed) formats.
    """
    counts = Counter()
    ...
    try:
        ...
        for raw in ds:
            ...
            if len(ids) == 0:
                # Roboflow format (1-indexed → 0-indexed)
                ids = tf.sparse.to_dense(
                    parsed["image/object/class/label"]
                ).numpy() - 1
            for i in ids:
