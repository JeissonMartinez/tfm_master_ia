"""Lanzador unificado de Custom Jobs en Vertex AI.

Lee un YAML de configuración, determina la familia del modelo
(YOLO → contenedor PyTorch, MobileNet → contenedor TensorFlow),
empaqueta el código fuente y lanza el entrenamiento.

Uso::

    python vertex_ai/launch_job.py \
        --config vertex_ai/configs/mobilenet_v3s_ssdlite_v1.yaml

    python vertex_ai/launch_job.py \
        --config vertex_ai/configs/yolo26n_v1.yaml \
        --run-name yolo26n_v1-ablation-lr

Requisitos previos:
    1. ``gcloud auth application-default login``
    2. El sdist ``tfm-trainer-1.0.0.tar.gz`` ya subido al bucket
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Lanza un Custom Training Job en Vertex AI"
    )
    parser.add_argument(
...
def main() -> None:
    ...
    # ── 2. Seleccionar contenedor y módulo ────────────────────────────
