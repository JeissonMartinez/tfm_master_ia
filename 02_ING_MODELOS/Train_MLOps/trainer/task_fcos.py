...
def main() -> None:
    ...
    logger.log_params({"model_size_mb": size_mb, **FCOS_SPECS})
    ...
    from src_colab import (
        ...
        IODCDataset, create_dataloader, iodc_collate_fn,
        build_fcos_loss,
    )
    ...
    batch_size = setup.common_config.get("batch_size", fc.get("batch_size", 16))
    ...
    train_cfg = TwoPhaseConfig(
        ...
        patience=setup.common_config.get("patience", 15),
    ...
    loss_fn = build_fcos_loss(
        ...
        centerness_weight=fc.get("centerness_weight", 1.0),
    )
    ...
    plot_training_curves(th, save_path=curves_path,
                         title=f"FCOS — {setup.experiment_name}")
    ...
    def fcos_predict_fn(images_tensor):
        return predict_fcos(model, images_tensor,
